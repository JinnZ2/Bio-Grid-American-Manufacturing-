  ...
  return (
    <div className="p-4 bg-gray-900 text-white rounded-lg shadow-md w-full md:w-1/3">
      ...
      <div className="text-sm space-y-2">
        <div>
          🧠 <strong>Convergence:</strong> {swarmMetrics.convergence.toFixed(1)}%
        </div>
        <div>
          📊 <strong>Efficiency:</strong> {swarmMetrics.efficiency.toFixed(1)}%
        </div>
        <div>
          📍 <strong>Discovered Nodes:</strong> {knowledgeNodes.filter((n) => n.discovered).length}{' '}
          / {knowledgeNodes.length}
        </div>
        <div>
          📡 <strong>Live Sensors:</strong> {sensors.length}
        </div>
      </div>
      ...
      <div className="space-y-1 max-h-48 overflow-y-auto text-xs">
        {sensors.map((s, idx) => (
          <div
            key={idx}
            className="bg-gray-800 px-2 py-1 rounded flex justify-between items-center"
          >
            <span>
              {s.type} @ {s.location}
            </span>
